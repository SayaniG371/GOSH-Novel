...
void sample_into_pool(VID_T* sample_array, int max_queue_size, int shuffle_base, long long samples_per_thread, 
long long samples_per_section, int samples_per_segment, int tid, int walk_length, int augmentation_distance, 
CSR<VID_T>* csr, seed* sd){
    // Allocate memory for private sample pool once per thread
    VID_T* private_sample_pool = new VID_T[samples_per_thread * 2];

    #pragma omp parallel for num_threads(2) schedule(static)
    for (long long current = 0; current < samples_per_thread; ++current) {
        VID_T* local_sample_pool = private_sample_pool + current * 2;
        VID_T source, p_sample;
        seed mySeed = *sd; // Each thread will work with its own copy of the seed

        // Start from a random node
        source = randn(&mySeed) % csr->num_vertices;

        // Perform the random walk
        for (int step = 0; step < walk_length; ++step) {
            p_sample = get_neighbor(csr->V, csr->E, source, randn(&mySeed)); // Get a random neighbor
            if (p_sample != UINT_MAX) {
                local_sample_pool[0] = source;
                local_sample_pool[1] = p_sample;
            } else {
                break;
            }
            source = p_sample; // Move to the next node
        }
    }

    // Copy the private sample pool to the main sample array
    for (int b = 0; b < shuffle_base; ++b) {
        memcpy(sample_array + b * samples_per_section * 2 + tid * samples_per_segment * 2, 
               private_sample_pool + samples_per_segment * b * 2, 
               2 * samples_per_segment * sizeof(VID_T)); 
    }

    // Deallocate memory for private sample pool
